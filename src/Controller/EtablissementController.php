...
class EtablissementController extends AbstractController
{
    ...
    public function new(Request $request, EtablissementRepository $etablissementRepository, UserPasswordHasherInterface $userPasswordHasher,LieuRepository $lieuRepository, UserRepository $userRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $lieuRepository->save($etablissement->getLieu(), true);
            $mdp = $etablissement->getUser()->getPassword();
            $etablissement->getUser()->setPassword(
                $userPasswordHasher->hashPassword(
                    $etablissement->getUser(),
                    $mdp
                )
            );
            $userRepository->save($etablissement->getUser(),true);
            $etablissement->getUser()->setRoles(array('ROLE_RES'));
